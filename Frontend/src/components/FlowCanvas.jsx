...
export function FlowCanvas({
  ...
  const onCanvasMouseDown = useCallback(
    (e) => {
      ...
      if (
        ...
        ["svg", "path", "rect", "circle"].includes(tag)
      ) {
        setSelected(null);
        panning.current = {
    ...
    (e) => {
      if (dragging.current) {
        ...
        setNodes((ns) =>
          ns.map((n) =>
            n.id === nodeId
              ? { ...n, x: e.clientX - startX, y: e.clientY - startY }
              : n,
          ),
        );
  ...
  const onCompleteConnect = useCallback(
    (targetId) => {
      ...
      setEdges((es) => {
        if (es.some((e) => e.from === connectingFrom && e.to === targetId))
          return es;
        return [
          ...es,
          { id: `e${++edgeIdCtr.current}`, from: connectingFrom, to: targetId },
        ];
      });
      setConnectingFrom(null);
    },
  ...
  const deleteEdge = useCallback(
    (edgeId) => setEdges((es) => es.filter((e) => e.id !== edgeId)),
    [],
  );
    ...
    setNodes((ns) =>
      ns.map((n) =>
        n.id === nodeId
          ? {
              ...n,
              fields: n.fields.map((f, i) => (i === fi ? { ...f, value } : f)),
            }
          : n,
      ),
    );
  ...
  useEffect(() => {
    if (openEditorNodeId != null) {
      setEditorNodeId(openEditorNodeId);
      onOpenEditorHandled?.();
    }
  }, [openEditorNodeId, onOpenEditorHandled]);

  const handleEditorSave = useCallback(
    async (nodeId, sysVal, userVal, saveName) => {
      setNodes((ns) =>
        ns.map((n) => {
          ...
          return {
            ...
            fields: n.fields.map((f) => {
              if (
                f.type === "textarea" &&
                f.placeholder?.toLowerCase().includes("system")
              )
                return { ...f, value: sysVal };
              if (
                f.type === "textarea" &&
                f.placeholder?.toLowerCase().includes("user")
              )
                return { ...f, value: userVal };
              return f;
            }),
          };
        }),
        ...
        return {
          ...
          fields: n.fields.map((f) => {
            if (
              ...
              return { ...f, value: sp.system || f.value };
            if (
              f.type === "textarea" &&
              f.placeholder?.toLowerCase().includes("user")
            )
              return { ...f, value: sp.user || f.value };
            return f;
          }),
        };
      }),
  ...
  const handleDeleteSaved = useCallback(
    (id) => setSavedPrompts((sp) => sp.filter((p) => p.id !== id)),
    [],
  );
  ...
  const runPipeline = useCallback(async () => {
    if (serverStatus !== "ok") {
      setRunLog([
        {
          text: "⚠  Backend not connected. Start the server first.",
          color: "warn",
        },
      ]);
      setShowLog(true);
    ...
    try {
      ...
      for await (const event of readSSE(resp)) {
        ...
        if (event.type === "node_start") {
          ...
          setRunLog((l) => [...l, { text: `→  Running: ${event.label}` }]);
        }
        ...
        if (event.type === "node_done") {
          ...
          setRunLog((l) => [
            ...l,
            {
              text: `✓  Done: ${(event.output || "").slice(0, 80)}`,
              dim: true,
            },
          ]);
        }
        if (event.type === "flow_done") {
          ...
          setRunLog((l) => [...l, { text: "✓  Flow complete" }]);
        }
        if (event.type === "error") {
          setNodeRunStatus((s) => ({ ...s, [event.nodeId]: "error" }));
          setRunLog((l) => [...l, { text: `✗  ${event.error}`, color: "err" }]);
        }
      }
    } catch (err) {
  ...
  const editorNode = editorNodeId
    ? nodes.find((n) => n.id === editorNodeId)
    : null;
  ...
  if (connectingFrom) {
    ...
    if (fn) {
      ...
      pendingPath = (
        <path
          d={makePath(from.x, from.y, dragPos.x, dragPos.y)}
          fill="none"
          stroke={t.text}
          strokeWidth="1.5"
          strokeDasharray="6 4"
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }} />
        <span
          style={{ fontSize: 10, color: t.textDim, fontFamily: "monospace" }}
        >
          drag right handle → left to connect · click × on edge to remove
        </span>
      </div>
            ...
            {nodes.map((n) => (
              ...
              >
                <FlowNode
                  ...
                  runStatus={nodeRunStatus[n.id] || null}
                  theme={t}
                />
              </div>
            ))}
...
export function FlowCanvasWrapper({
  ...
  return (
    <FlowCanvas
      ...
      openEditorNodeId={openEditorNodeId}
      onOpenEditorHandled={onOpenEditorHandled}
    />
  );
}

export function FlowRunButton({ theme: t }) {
  return (
    <button
